...
fn draw_known_peers_time(f: &mut Frame, area: Rect, data: &NodeData) {
    ...
    // Bucket addresses by network and time
...
fn service_bit_name(bit: u8) -> &'static str {
    match bit {
        ...
        3 => "NODE_WITNESS", 6 => "NODE_COMPACT_FILTERS", 10 => "NODE_NETWORK_LIMITED",
        24 => "NODE_P2P_V2", 27 => "NODE_REDUCED_DATA", _ => "",
    }
}
...
fn draw_known_peers_services(f: &mut Frame, area: Rect, data: &NodeData, scroll: u16) {
    let local_services: u64 = u64::from_str_radix(
        data.network.localservices.trim_start_matches("0x"), 16,
    ).unwrap_or(0);
    ...
    for bit in 0..64u8 {
        if all_bits_mask & (1u64 << bit) != 0 { active_bits.push(bit); }
    }
    ...
    for addr in &data.known_addresses {
        ...
        for (i, &bit) in active_bits.iter().enumerate() {
            if addr.services & (1u64 << bit) != 0 { entry.1[i] += 1; }
        }
    }
